...
ISR(USART0_RX_vect){
	...
	if(data == '\r' || data == '\n'){
		rx_Quebuff[input_index][i] = '\0'; //문장의 끝을 알리는 null sign을 insert함.
...
void pc_command_processing()
{
	...
	if( output_index != input_index){
		cmd = rx_Quebuff[output_index]; //&rx_buff[front][0]로 // 2차원 배열의 0번째 주소를 넘겨줌
		printf("%s\n",cmd);
